...
def load_yolov7_model(weight, device='cpu'):
    ...
    model = Model("/home/cqx/quant/yolov7/cfg/training/yolov7.yaml", ch=3, nc=80).to(device)
...
def evaluate_coco(model, loader, save_dir='.', conf_thres=0.001, iou_thres=0.65):
    ...
    return test.test(
        "/home/cqx/quant/yolov7/data/coco.yaml",
...
if __name__ == "__main__":
    weight = '/home/cqx/quant/yolov7.pt'
    pth=load_yolov7_model(weight, torch.device('cuda:0'))
    replace_to_quantization_model(pth)
    model = prepare_model(weight, torch.device('cuda:0'))
    qunant_model=replace_to_quantization_model(model)
    dataloader = prepare_val_dataset("/home/cqx/quant/dataset/coco2017")
    evaluate_coco(model, dataloader, save_dir='./results')