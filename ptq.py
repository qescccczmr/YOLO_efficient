...
if __name__ == "__main__":
    ...
    parser.add_argument("--eval_origin", action="store_true", default=True,help="do eval for origin model")
    parser.add_argument("--eval_ptq", action="store_true",default=True, help="do eval for ptq model")
    parser.add_argument("--conf_thres", type=float, default=0.001, help="confidence threshold")
    parser.add_argument("--iou_thres", type=float, default=0.65, help="nms threshold")
    ...
    # PTQ 量化
    ignore_layers= ["model\.104\.m\.(.*)", "model\.101\.m\.(.*)","model\.96\.m\.(.*)", "model\.102\.m\.(.*)","model\.63\.m\.(.*)", "model\.103\.m\.(.*)","model\.99\.m\.(.*)", "model\.18\.m\.(.*)"
                    ,"model\.18\.m\.(.*)", "model\.72\.m\.(.*)",]
